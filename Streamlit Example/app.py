...
st.title("Grievnces Text Analytics")
...
st.subheader("Title: Frequency Distribution of Long Duration Trigrams")
wordfrequency = frequency_generator(filtereddata['CUST_FEEDBACK'])
duration_ngrams= wordfrequency[:30]
duration_freq=pd.DataFrame(duration_ngrams[0:14],columns=["Trigram","Frequency"])
fig = plt.figure()
